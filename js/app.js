...
const ETIQUETAS_FILTRO = {
  ...
  responsable: "Responsable", tipo: "Tipo de incidencia", gravedad: "Gravedad",
};
const CAMPO_OPCIONES = {
  ...
  responsable: "responsables", tipo: "tipos", gravedad: "gravedades",
};
...
function pintarGraficas(graficas) {
  ...
  const orden = ["por_tipo", "por_gravedad", "por_cliente", "por_estado",
...
function pintarDetalle(incidencias) {
  ...
  }).join("") || `<tr><td colspan="12" class="text-center text-muted py-3">
  ...
  try {
    ...
    if (archivos.length) {
      ...
      if (r.rechazadas?.length) {
        avisar(`Archivos rechazados: ${r.rechazadas.join(", ")}`, "warning");
      }
